<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="ai.jobiak.model.Product,java.util.ArrayList,java.sql.*"%>
<!DOCTYPE html>
<html>
<head>
...
<% 
		...
		for(int i=0;i<itemsList.size();i++){
			out.println("<a href='deleteitems.jsp?item="+itemsList.get(i).getProductId()+"'>Add Items  :"+itemsList.get(i).getProductId()+"</a></br>");
		}
		...
		shoppingCart.setAttribute("items", selectedItemsList);
	     ...
	     for(int i=0;i<selectedItemsList.size();i++){
				out.println("<a href='deleteitems.jsp?item="+selectedItemsList.get(i).getProductId()+"'>Delete Items  :"+selectedItemsList.get(i).getProductId()+"</a></br>");
			}
		     if (request.getQueryString()!=null) //to avoid NullPointer Exception
		      {
					...
					String ProductId=splitArray[1];
			   for(int i=0;i<selectedItemsList.size();i++) {
		     if(selectedItemsList.get(i).getProductId().equals(ProductId)) {
			selectedItemsList=(ArrayList<Product>)shoppingCart.getAttribute("items");
		selectedItemsList.remove(selectedItemsList.get(i));	
			shoppingCart.removeAttribute("selectedItemsList"); 
			}
		      }
		      }
